...
import { httpPostSearchPostsNLP } from "./requests";

function useSearchPostsNLP(searchQueryValue) {
    ...
    useEffect(() => {
        const fetchPosts = async () => {
            const fetchedRelevantPosts = await httpPostSearchPostsNLP(searchQueryValue, numCandidates, limit);
            setRelevantPosts(fetchedRelevantPosts);
        };

        if (searchQueryValue) { // Ensure there's a query to search for.
            fetchPosts();
        }
    }, [searchQueryValue]);

    return relevantPosts; // The hook now only needs to return the state, not the setter.
}
export default useSearchPostsNLP;